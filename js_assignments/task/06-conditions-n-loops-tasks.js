 ...
 */
 function isInsideCircle(circle, point) {
    let fromCenterToPoint = Math.hypot(point.x - circle.center.x, point.y - circle.center.y);
 ...
 */
 function isCreditCardNumber(ccn) {
    let ccnArr = String(ccn).split('');
    ccnArr = ccnArr.map(item => +item);
    let i = (ccnArr.length % 2 === 0) ? 0 : 1;

    for (i; i < ccnArr.length; i+=2) {
        if (ccnArr[i]*2 > 9) {
    ...
    let sum = ccnArr.reduce((sum, item) => sum + item);
 ...
 */
 function getDigitalRoot(num) {
    var arrN = num.toString().split('');
    var sum;
    while (arrN.length > 1) {
        sum = arrN.reduce(function(sum, item) { return sum + +item; }, 0);
